...
def compare_guess(guess: dict, target: dict) -> dict:
    ...
    return {
        "artist": {"value": guess["artist"], "status": "correct" if guess["id"] == target["id"] else "wrong", "hint": ""},
        "genres": {"value": ", ".join(guess["genres"][:3]) or "Unknown", "status": genre_status, "hint": genre_hint},
        "popularity": {"value": guess["popularity"], "status": popularity_status, "hint": popularity_hint},
        "followers": {"value": guess["followers"], "status": followers_status, "hint": followers_hint},
    }
...
with st.sidebar:
    ...
    st.write("Target is fixed for testing.")
